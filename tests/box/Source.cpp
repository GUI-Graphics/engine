...
int main()
{
	...
	while (window.active())
	{
		...
		glUniformMatrix4fv(modelMatrixLocation, 1, GL_TRUE, cubeLocation->matrix.elements);
		...
		glBindVertexArray(box.vao);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, box.index);
		glDrawElements(GL_TRIANGLES, (GLsizei)box.indices.size(), GL_UNSIGNED_INT, 0);
